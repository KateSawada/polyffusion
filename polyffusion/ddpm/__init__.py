...
class SequenceDenoiseDiffusion(nn.Module):
    """
    ## Denoise Diffusion
    """
    ...
    def loss(self, x0: torch.Tensor, mask: torch.Tensor, noise: Optional[torch.Tensor] = None):
        """
        #### Simplified Loss
        """
        # Get batch size
        batch_size = x0.shape[0]
        x0 = x0.unsqueeze(1)  # adjust shape for q_sample
        # Get random $t$ for each sample in the batch
        t = torch.randint(
            0, self.n_steps, (batch_size,), device=x0.device, dtype=torch.long
        )
